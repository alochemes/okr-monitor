...
| 2026-04-29 (late) | **Strategic reframe** captured at session end (operator observation): "we don't have OKRs and KPIs either." The wedge is the **OKR/KPI framework installation**, not just the agent tooling. Customer-facing playbook (`notion/03_playbooks/01_customer_okr_kpi_framework.md`) elevated from supporting documentation to lead deliverable of every pilot. Agents become the delivery mechanism for the framework, not the product itself. | Companies don't need another tool — they need someone to install a framework that works, then a system that watches the work against it. The 5-in-5 exercise + the new pilot intake questionnaire + this playbook are the artifacts that DO the cleanup. The agents make the cleanup stick. | — |
| 2026-04-29 (late) | Added our own KPI section to `TRACKER.md §3.5` (10 KPIs covering cost discipline, report delivery, test pass rate, web build health, balance runway, audit-log integrity). Distinct from OKRs (which drive change). KPIs should be green continuously; alerts on red. | Eat our own food. We were operating without a KPI layer ourselves while building a product that monitors KPIs. K1 / K4 / K5 / K8 / K10 are green right now; the others baseline as the relevant cycles fire. | — |
| 2026-04-29 (late) | Pilot intake questionnaire shipped (`notion/03_playbooks/04_pilot_intake_questionnaire.md`). 30-question 15-minute fillable doc covering company basics, current OKR/KPI practice, integrations, rituals, the 5-in-5 exercise, and pilot specifics. Sent BEFORE the kickoff call — answers shape the customized kickoff agenda and the OKR Health Check we generate. | First customer-facing artifact in the framework-installation flow. The 5-in-5 output is the highest-signal input we'll ever get from a pilot — it surfaces the real KRs faster than any framework discussion. | — |
