...
- **`web/`** — marketing landing page (Next.js 15 + Tailwind, deployed to Vercel). Editorial design, single accent color, mock printed memo as the hero asset. Includes PostHog A/B testing on hero headline + CTA. See `web/README.md`.
- **`notion/`** — knowledge base in markdown, drag-importable into Notion. Includes the customer-facing OKR/KPI framework (the wedge), KB-mining playbook, 60-day pilot kickoff, social media setup. See `notion/00_MASTER_INDEX.md`.
- **`reports/`** — committed daily OWNER/FINANCE briefs + A/B reports (auto-generated by routines).
- **`proposals/`** — committed weekly strategy-pod briefs + per-agent proposals (auto-generated Sundays).
