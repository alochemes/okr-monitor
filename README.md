...
The product surface (a Next.js web app) is not yet built — it's Sprint 0. The agent system is the brain; the web app will be the customer surface.
...
- **Product/Design** (3/5): `pm` · `ux_researcher` · `copywriter` · _ux_designer, ui_designer pending_
- **GTM** (3/6): `founder_sales` · `demand_gen` · `content` · _growth_hacker, sales_engineer, community_pr pending_
- **Customer/Ops** (3/4): `pilot_pm` · `onboarding` · `analytics_ops` · _support pending_
- **Engineering** (0/7): all pending — these pair with Claude Code on actual code, not background daily runs
...
- 16/30 agents live
- 4 commits on `main`
- 2 remote routines scheduled (Sunday 6pm Pacific + Daily 7pm Pacific)
- 9/9 math tests passing
- API workspace alignment resolved (funded key in .env, override=True everywhere)
- First real CEO proposal generated for $0.017 — quality verified
- MVP web app: not started · Engineering pod: not scaffolded

See `TRACKER.md` for the full state.
