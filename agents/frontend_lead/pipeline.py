"""Frontend-Lead pipeline. One run = one ui_architecture proposal."""
...
_AGENT = "frontend_lead"
_KIND = "ui_architecture"
_PROMPT = (Path(__file__).parent / "prompts" / "ui_architecture.md").read_text(encoding="utf-8")


def _user_message(today: date) -> str:
    return (
        f"Today is {today.isoformat()}.\n\n"
        "Spec the UI architecture for the dashboard MVP — KR scoreboard table + drilldown panel. "
...
def run(*, today: date | None = None) -> dict[str, Any]:
    return run_proposal(
        ...
        body_md_fn=lambda p, t: render_dict_as_markdown(
            p, t, list_keys=("component_tree", "state_model"),
        ),
        today=today,
    )
